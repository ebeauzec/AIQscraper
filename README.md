...
| **No Reference Library enrichment** — you must manually cross-reference EOA lists, firmware baselines, and MetroCluster ISL specs | **Automatic enrichment from 7 external sources** — EOA hardware flags, firmware baseline checks, MetroCluster ISL validation, Kerberos AES detection, SnapMirror policy audit, KB articles with remediation steps, upgrade path recommendations, and best practice TR guides. All scraped from docs.netapp.com, security.netapp.com, NVD, kb.netapp.com, and Bugs Online. Persistently cached in SQLite with automatic refresh. |

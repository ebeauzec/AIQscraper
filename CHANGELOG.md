...
### Fixed
- **Contracts & Lifecycle Events tile counts computed from a different data source than the table below them.** The tiles only read `contracts.endDate`, while the table correctly reads Active IQ's `lifecycleEvents` feed — a scope with empty `contracts.endDate` but real CRITICAL "Contract Expiration" events at 13 days out (visible in the table) showed "Expiring < 90 Days: 0" on the tile directly above it. Tile counts now union both sources.

### Added
- **"Generated By: AIQ Advisor vX.X.X" in the Section 12 TXT export header**, so a re-shared report makes clear whether it predates a fix. Used to confirm live that the 5.6.12 recommendations account-scoping fix works correctly against real data (37 → 23 recs, zero duplicates) — a report still showing the old duplicated behavior was generated by a browser tab that hadn't reloaded since before that fix shipped.

...
let state = {
  ...
  searchTerm: "",          // Customer/account search term for API discovery (e.g. 'Vodacom South Africa')
...
function loadConfig() {
  ...
  state.searchTerm    = safeGetItem("aiq_search_term") || "";
...
async function runAPIDiagnostics() {
  ...
  try {
    ...
    // ── System search by KNOWN SERIAL NUMBERS ───────────────────────────────
    const knownSerials = [
      "211839000195",   // Confirmed registered (Vodacom South Africa)
      "952239002356",   // Cluster node serial
      "952239002659",
      "952239002236",
      "952239002406"
    ];
    ...
    for (const sn of knownSerials) {
      await probe(`Cluster view: ${sn}`,     `/v1/clusterview/get-cluster-summary/${sn}`, true);
      await probe(`System v3 search: ${sn}`, `/v3/search/system?searchText=${sn}&limit=10`);
      await probe(`System v1 search: ${sn}`, `/v1/search/system/level/customer?searchText=${sn}&limit=10`);
      await probe(`EoS serial: ${sn}`,       `/eos/details?serialNumbers=${sn}`);
      await probe(`Health sys: ${sn}`,       `/v1/health/summary/level/system/id/${sn}`, true);
    }
    // Also try cluster resolver for the first serial (most complete info)
    await probe(`Cluster resolver: 211839000195`, `/v1/clusterview/resolver/211839000195`, true);
    ...
    await probe("Upgrade (customer, Vodacom)", `/upgrade/summary/level/customer?searchText=${diagTermEnc}`);
...
async function saveSettings() {
  ...
  const searchTermInput = (document.getElementById("settingsSearchTerm")?.value || "").trim();
...
async function loadProductionData() {
  ...
  // Helper: robustly extract a list from any API response shape
  function extractList(data) {
    if (!data || typeof data !== "object") return [];
    if (Array.isArray(data)) return data;
    for (const key of ["results","data","hits","items","records",
                        "systemDetailsByInventory","systemList","systems",
                        "customers","watchListData","watchlists",
                        "customerList","aggregates","clusterDetails"]) {
  ...
  try {
    ...
    state.watchlists = wlList.map(wl => {
      ...
      return { id: wlId, name: wl.watchListName || wl.watchlistName || wl.name || "Watchlist" };
    }).filter(w => w.id);
    saveWatchlists();
    console.log(`[AIQ] Watchlists: ${watchlistIds.length} IDs`);
  } catch (e) { console.warn("[AIQ] /watchlist/all:", e.message); }

  // ── Step 2: Resolve customer IDs from search term ────────────────────────
  if (textLabel) textLabel.innerText = term ? `Searching for "${term}"...` : "Discovering Customers...";
  let customerIds = [];

  if (term) {
    for (const ep of [
      `/v3/search/aggregate?searchText=${termEnc}&limit=200`,
      `/v1/search/aggregate/level/customer?searchText=${termEnc}&limit=200`,
      `/v3/search/parent?searchText=${termEnc}&limit=200`,
      `/v1/search/parent/level/customer?searchText=${termEnc}&limit=200`,
      `/v3/search/location?searchText=${termEnc}&limit=200`,
    ]) {
      try {
        const data = await callActiveIQAPI(ep);
        const list = extractList(data);
        list.forEach(c => {
          const id = c.customerId || c.id || c.aggregateId || c.parentId || c.locationId;
          if (id) customerIds.push(String(id));
        });
        if (customerIds.length > 0) {
          console.log(`[AIQ] ${ep} → ${customerIds.length} customer IDs`);
          break;
        }
      } catch (e) { console.warn(`[AIQ] aggregate ${ep}:`, e.message); }
    }
  }

  // ── Step 3: System search ────────────────────────────────────────────────
  if (textLabel) textLabel.innerText = "Loading System Inventory...";
  let systemsList  = [];
  let usedEndpoint = null;

  // 3a. By customer ID + term
  for (const custId of customerIds.slice(0, 30)) {
    for (const searchStr of [term ? termEnc : "*", "*"]) {
      try {
        const ep   = `/v1/search/system/level/customer?id=${custId}&searchText=${searchStr}&limit=1000`;
        const data = await callActiveIQAPI(ep);
  ...
  // 3b. Direct system search by term (no ID needed)
  if (systemsList.length === 0 && term) {
    for (const ep of [
      `/v1/search/system/level/customer?searchText=${termEnc}&limit=1000`,
      `/v2/search/system/level/customer?searchText=${termEnc}&limit=1000`,
      `/v3/search/system?searchText=${termEnc}&limit=1000`,
    ]) {
      try {
        const data = await callActiveIQAPI(ep);
        const page = extractList(data);
        if (page.length > 0) { systemsList = page; usedEndpoint = ep; break; }
      } catch (e) { console.warn(`[AIQ] system search ${ep}:`, e.message); }
    }
  }
  ...
  if (systemsList.length === 0) {
    for (const wlId of watchlistIds) {
      try {
        const ep   = `/v1/search/system/level/watchlist?id=${wlId}&searchText=${term ? termEnc : "*"}&limit=1000`;
        const data = await callActiveIQAPI(ep);
        const page = extractList(data);
        if (page.length > 0) { systemsList.push(...page); usedEndpoint = ep; }
      } catch (e) { /* skip */ }
  ...
  // 3d. Serial bootstrap — use known serials to discover customer ID, then pull full fleet
  // Works even when /watchlist/all is broken. Cluster view returns customerId we can reuse.
  if (systemsList.length === 0) {
    if (textLabel) textLabel.innerText = "Serial Bootstrap Discovery...";
    const seedSerials = [
      "211839000195", "952239002356", "952239002659", "952239002236", "952239002406"
    ];
    for (const sn of seedSerials) {
      try {
        const clData = await callActiveIQAPI(`/v1/clusterview/get-cluster-summary/${sn}`);
        console.log(`[AIQ] Cluster view ${sn}:`, JSON.stringify(clData).slice(0, 300));
        // Extract customer ID from cluster summary
        const custId = clData?.customerId || clData?.customer_id ||
                       clData?.customerInfo?.customerId || clData?.clusterInfo?.customerId;
        if (custId) {
          console.log(`[AIQ] Bootstrap found customerId=${custId} via serial ${sn}`);
          // Now get ALL systems for this customer
          for (const ver of ["v1", "v2"]) {
            try {
              const ep   = `/${ver}/search/system/level/customer?id=${custId}&searchText=*&limit=1000`;
              const data = await callActiveIQAPI(ep);
              const page = extractList(data);
              if (page.length > 0) { systemsList = page; usedEndpoint = ep; break; }
            } catch (_) { /* skip */ }
        ...
        // Even if no customer ID, the cluster summary itself is a system record
        if (systemsList.length === 0 && clData && Object.keys(clData).length > 0) {
          systemsList = [clData];
          usedEndpoint = `/v1/clusterview/get-cluster-summary/${sn}`;
        }
      } catch (e) { console.warn(`[AIQ] cluster bootstrap ${sn}:`, e.message); }
      if (systemsList.length > 0) break;
    }
  }

  // 3e. Breadth endpoints (return data for ALL accessible systems — no ID needed)
  if (systemsList.length === 0) {
    for (const ep of ["/eos/details", "/eos/contracts/details",
                       "/v2/health/risks", "/capacity2/usage",
                       "/firmware/details",
                       `/v1/health/summary/level/customer?searchText=${term ? termEnc : "*"}&limit=1000`,
                       `/v1/health/details/level/customer?searchText=${term ? termEnc : "*"}&limit=1000`]) {
      try {
        const data = await callActiveIQAPI(ep);
        const page = extractList(data);
        if (page.length > 0) { systemsList = page; usedEndpoint = ep; break; }
      } catch (e) { console.warn(`[AIQ] breadth ${ep}:`, e.message); }
    }
  }
  ...
  if (systemsList.length === 0) {
    ...
    const termHint = term
      ? `Search term used: "${term}"\n  (Change in Settings → Customer / Account Search Term)`
      : `No search term configured.\n  Go to Settings → Customer / Account Search Term\n  and enter your customer name (e.g. "Vodacom South Africa")`;

    alert(
      "Active IQ API: Connected ✓   Authentication: Valid ✓   Data: Empty\n\n" +
      termHint + "\n\n" +
      "Tip: Run 'API Diagnostics' to see full endpoint status.\n" +
      "Tip: Check activeiq.netapp.com to confirm systems are visible in the web portal."
    );
    updateStatusIndicators();
    return;
  }

  // ── Step 5: Normalise & store ────────────────────────────────────────────
  const normalise = s => ({
...
ync function loadProductionData() {
  const textLabel = document.getElementById("connectionStatusText");
  const indicator  = document.querySelector(".indicator");
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // CONFIRMED endpoint catalog (activeiq.netapp.com/catalog/partner/api-reference)
  //
  //  Watchlists (NOT in public catalog but confirmed via GitHub library):
  //    GET /v1/watchlist/all          → { watchListData:[{ watchListId, watchListName }] }
  //
  //  System SEARCH (in catalog — this is the correct way to list systems):
  //    GET /v3/search/system          → all accessible systems, no level/id needed
  //    GET /v2/search/system/level/{level}
  //    GET /v1/search/system/level/{level}
  //    (level: customer | watchlist | site | group | partner)
  //    Query params: searchText, limit, offset
  //
  //  Breadth endpoints (no level/id → return for ALL accessible systems):
  //    GET /v1/eos/details            → EoS system list (serial, model, contract dates)
  //    GET /v2/health/risks           → health risks (serial, customer, cluster info)
  //    GET /v1/health/summary/level/{level}/id/{id}
  //    GET /v1/health/details/level/{level}/id/{id}
  //
  //  NOTE: /v1/system/inventory/... does NOT exist in the public catalog.
  //        The previous calls to that path all returned 404.
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    const raw = await callActiveIQAPI("/watchlist/all");
    if (raw) {
      const wlList = Array.isArray(raw) ? raw : (raw.watchListData || raw.watchlists || []);
      state.watchlists = wlList.map(wl => {
        const serials = wl.serialNumbers || wl.systemSerials || wl.systems || [];
        serials.forEach(sn => watchlistSerials.add(String(sn).trim()));
        const wlId = wl.watchListId || wl.watchlistId || wl.id;
        if (wlId) watchlistIds.push(String(wlId));
        return {
          id:            wlId || "wl_" + Date.now(),
          name:          wl.watchListName || wl.watchlistName || wl.name || "Watchlist",
          systemSerials: serials
        };
      });
      saveWatchlists();
      console.log(`[AIQ] Watchlists: ${state.watchlists.length} | IDs: [${watchlistIds.join(", ")}]`);
    }
  } catch (e) {
    console.warn("[AIQ] /watchlist/all:", e.message);
  }
  ...
  // Helper: extract a systems array from any search/discovery response
  function extractSystems(data) {
    if (!data) return [];
    if (Array.isArray(data)) return data;
    // Try every known response key the catalog endpoints use
    return data.systemDetailsByInventory
        || data.hits
        || data.results
        || data.systems
        || data.systemList
  ...
  // ── 2a. Search API — v3 (no level needed, widest scope) ──────────────────
  // GET /v3/search/system
  try {
    const data = await callActiveIQAPI("/v3/search/system?searchText=&limit=1000");
    const page = extractSystems(data);
    console.log("[AIQ] /v3/search/system →", JSON.stringify(Object.keys(data || {})));
    if (page.length > 0) {
      systemsList = page;
      usedEndpoint = "/v3/search/system";
    }
  } catch (e) { console.warn("[AIQ] /v3/search/system:", e.message); }

  // ── 2b. Search API — v2/v1 with watchlist level ───────────────────────────
  if (systemsList.length === 0 && watchlistIds.length > 0) {
    for (const wlId of watchlistIds) {
      for (const ver of ["v2", "v1"]) {
        try {
          const ep = `/${ver}/search/system/level/watchlist?id=${wlId}&limit=1000`;
          const data = await callActiveIQAPI(ep);
          const page = extractSystems(data);
          console.log(`[AIQ] ${ep} → keys:`, Object.keys(data || {}), "count:", page.length);
          if (page.length > 0) {
            systemsList.push(...page);
            usedEndpoint = ep;
          }
        } catch (e) { console.warn(`[AIQ] /${ver}/search/system watchlist ${wlId}:`, e.message); }
      }
      if (systemsList.length > 0) break;
    }
  }

  // ── 2c. Search API — customer level ──────────────────────────────────────
  if (systemsList.length === 0) {
    for (const ver of ["v2", "v1"]) {
      try {
        const ep = `/${ver}/search/system/level/customer?searchText=&limit=1000`;
        const data = await callActiveIQAPI(ep);
        const page = extractSystems(data);
        console.log(`[AIQ] ${ep} → keys:`, Object.keys(data || {}), "count:", page.length);
        if (page.length > 0) { systemsList = page; usedEndpoint = ep; break; }
      } catch (e) { console.warn(`[AIQ] /${ver}/search/system/level/customer:`, e.message); }
    }
  }

  // ── 2d. EoS details — no level/id, returns all accessible systems ─────────
  // GET /v1/eos/details  — contains serial, model, EoS dates, customer info
  if (systemsList.length === 0) {
    try {
      const data = await callActiveIQAPI("/eos/details");
      const page = extractSystems(data);
      console.log("[AIQ] /v1/eos/details → keys:", Object.keys(data || {}), "count:", page.length);
      if (page.length > 0) { systemsList = page; usedEndpoint = "/eos/details"; }
    } catch (e) { console.warn("[AIQ] /eos/details:", e.message); }
  }

  // ── 2e. Health risks — no level/id, all accessible systems ───────────────
  // GET /v2/health/risks
  if (systemsList.length === 0) {
    try {
      const data = await callActiveIQAPI("/v2/health/risks");
      const page = extractSystems(data);
      console.log("[AIQ] /v2/health/risks → keys:", Object.keys(data || {}), "count:", page.length);
      if (page.length > 0) { systemsList = page; usedEndpoint = "/v2/health/risks"; }
    } catch (e) { console.warn("[AIQ] /v2/health/risks:", e.message); }
  }

  // ── 2f. Health summary per watchlist ─────────────────────────────────────
  // GET /v1/health/details/level/watchlist/id/{watchlistId}
  if (systemsList.length === 0 && watchlistIds.length > 0) {
    for (const wlId of watchlistIds) {
      try {
        const data = await callActiveIQAPI(`/health/details/level/watchlist/id/${wlId}`);
        const page = extractSystems(data);
        console.log(`[AIQ] /health/details watchlist ${wlId} → keys:`, Object.keys(data || {}), "count:", page.length);
        if (page.length > 0) {
          systemsList.push(...page);
          usedEndpoint = `/health/details/level/watchlist/id/${wlId}`;
        }
      } catch (e) { console.warn(`[AIQ] /health/details watchlist ${wlId}:`, e.message); }
    }
  }

  // ── 2g. EoS summary (may reveal system-level data) ───────────────────────
  if (systemsList.length === 0) {
    try {
      const data = await callActiveIQAPI("/eos/summary");
      const page = extractSystems(data);
      if (page.length > 0) { systemsList = page; usedEndpoint = "/eos/summary"; }
    } catch (e) { console.warn("[AIQ] /eos/summary:", e.message); }
  }

  // ── Step 3: No systems found ──────────────────────────────────────────────
  if (systemsList.length === 0) {
    if (textLabel) textLabel.innerText = "No Systems Found";
    if (indicator)  indicator.className = "indicator disconnected";
    const wlNote = watchlistIds.length > 0
      ? `Watchlist IDs found: ${watchlistIds.join(", ")}`
      : "No watchlists found for this token.";
    alert(
      "Active IQ API is connected and authenticated, but returned no systems.\n\n" +
      wlNote + "\n\n" +
      "Endpoints tried (check browser console for raw responses):\n" +
      "  /v3/search/system\n" +
      "  /v2+v1/search/system/level/watchlist\n" +
      "  /v2+v1/search/system/level/customer\n" +
      "  /v1/eos/details\n" +
      "  /v2/health/risks\n" +
      "  /v1/health/details/level/watchlist/id/{id}\n\n" +
      "Most likely cause: the API token scope does not include fleet/system data.\n" +
      "Try generating a new token in: activeiq.netapp.com → Quick Links → API Services"
    );
    updateStatusIndicators();
    return;
  }

  // ── Step 4: Normalise & store ─────────────────────────────────────────────
  // Field names vary by endpoint — normalise to a consistent internal shape
  const normalise = s => ({
    serialNumber:    s.serialNumber    || s.serial_number    || s.systemSerialNumber || "",
    systemName:      s.systemName      || s.system_name      || s.hostName          || s.clusterName || "",
    model:           s.model           || s.platformType     || s.platform          || "",
    ...
    warningCount:    Number(s.warningCount || s.warning_count || 0),
    criticalCount:   Number(s.criticalCount|| s.critical_count|| 0),
    ...s  // preserve all raw fields for enrichment functions
  });

  // If watchlist-only mode is on and we have serial numbers from watchlists,
  // filter to only those systems (when we couldn't query by watchlist ID directly)
  if (state.watchlistOnly && watchlistSerials.size > 0 && usedEndpoint !== null &&
  ...
  safeSetItem("aiq_last_sync", state.lastSync);
  if (state.systems.length > 0) state.selectedSystem = state.systems[0];
  console.log(`[AIQ] Sync complete via '${usedEndpoint}': ${state.systems.length} systems.`);
  updateStatusIndicators();
  refreshUIState();
}
...
function switchTab(tabId) {
  ...
  if (tabId === "overview") {
    ...
    if (document.getElementById("settingsSearchTerm")) document.getElementById("settingsSearchTerm").value = state.searchTerm || "";
    updateScheduledSyncInfo();
  }
}

...
async function loadProductionData() {
  ...
  // ── CONFIRMED NetApp API endpoint structure (from woutercoppens/netapp-activeiq-api) ──
  //
  //  GET /v1/watchlist/all
  //      → returns all watchlists for the authenticated user (includes watchListId)
  //
  //  GET /v1/system/inventory/details/level/{level}/id/{id}[?start=0&limit=100]
  //      → {level}: customer | site | group | watchlist | serialNumbers
  //      → {id}:    customerId | siteId | groupName | watchlistId | serialNumber(s)
  //
  //  GET /v1/system/list/level/{level}
  //      → {level}: customers | sites | groups
  //      → returns the top-level org list (no id needed)
  //
  // Response shape: { systemDetailsByInventory: [ { serialNumber, systemName,
  //   clusterName, customerName, customerId, platform, osVersion, ... } ] }

  // ── Step 1: Fetch all watchlists ──────────────────────────────────────────
  ...
  try {
    ...
    if (raw) {
      const wlList = Array.isArray(raw)
        ? raw
        : (raw.watchListData || raw.watchlists || []);
    ...
    console.warn("[AIQ] /watchlist/all failed:", wlErr.message);
  }

  // Helper: fetch paginated inventory from a single confirmed endpoint
  // GET /v1/system/inventory/details/level/{level}/id/{id}
  async function fetchInventory(level, id) {
    const pageSize = 100;
    let allSystems = [];
    let start = 0;
    while (true) {
      const ep = `/system/inventory/details/level/${level}/id/${id}?start=${start}&limit=${pageSize}`;
      const data = await callActiveIQAPI(ep);
      // Response key can be systemDetailsByInventory or systemList or similar
      const page = data?.systemDetailsByInventory
        || data?.systemList
        || data?.systems
        || (Array.isArray(data) ? data : []);
      allSystems = allSystems.concat(page);
      console.log(`[AIQ] ${ep} → ${page.length} systems (total ${allSystems.length})`);
  ...
  // ── Step 2: Fetch system inventory ────────────────────────────────────────
  if (textLabel) textLabel.innerText = "Loading System Inventory...";
  let systemsList  = [];
  let usedEndpoint = null;

  // 2a. Watchlist-scoped (preferred when watchlist IDs available)
  //     Confirmed path: GET /v1/system/inventory/details/level/watchlist/id/{watchlistId}
  if (watchlistIds.length > 0) {
    for (const wlId of watchlistIds) {
      try {
        const systems = await fetchInventory("watchlist", wlId);
        if (systems.length > 0) {
          systemsList.push(...systems);
          usedEndpoint = `/system/inventory/details/level/watchlist/id/${wlId}`;
          console.log(`[AIQ] Watchlist ${wlId}: ${systems.length} systems`);
        }
      } catch (e) {
        console.warn(`[AIQ] Watchlist inventory ${wlId} failed: ${e.message}`);
      }
    }
  }

  // 2b. Customer-level fallback: get list of customers, then inventory per customer
  //     GET /v1/system/list/level/customers → then
  //     GET /v1/system/inventory/details/level/customer/id/{customerId}
  if (systemsList.length === 0) {
    try {
      if (textLabel) textLabel.innerText = "Loading Customer Fleet...";
      const custData = await callActiveIQAPI("/system/list/level/customers");
      const customers = custData?.customers
        || custData?.customerList
        || (Array.isArray(custData) ? custData : []);
      console.log(`[AIQ] Customers found: ${customers.length}`);

      for (const cust of customers) {
        const custId = cust.customerId || cust.id;
        if (!custId) continue;
        try {
          const systems = await fetchInventory("customer", custId);
          systemsList.push(...systems);
          usedEndpoint = `/system/inventory/details/level/customer/id/${custId}`;
        } catch (e) {
          console.warn(`[AIQ] Customer ${custId} inventory failed: ${e.message}`);
        }
      }
    } catch (e) {
      console.warn("[AIQ] /system/list/level/customers failed:", e.message);
    }
  }

  // 2c. Last resort: try the site and group level lists
  if (systemsList.length === 0) {
    for (const listLevel of ["sites", "groups"]) {
      try {
        const listData = await callActiveIQAPI(`/system/list/level/${listLevel}`);
        const items = listData?.[listLevel]
          || listData?.siteList
          || listData?.groupList
          || (Array.isArray(listData) ? listData : []);
        for (const item of items) {
          const itemId = item.siteId || item.groupId || item.id;
          if (!itemId) continue;
          const level = listLevel === "sites" ? "site" : "group";
          try {
            const systems = await fetchInventory(level, itemId);
            systemsList.push(...systems);
            usedEndpoint = `/system/inventory/details/level/${level}/id/${itemId}`;
          } catch (e) { /* skip */ }
        }
        if (systemsList.length > 0) break;
      } catch (e) {
        console.warn(`[AIQ] /system/list/level/${listLevel} failed: ${e.message}`);
      }
    }
  }
  ...
  if (systemsList.length === 0) {
    ...
    alert(
      "Active IQ API connected but returned no systems.\n\n" +
      "Endpoints tried:\n" +
      "  • /v1/watchlist/all → /v1/system/inventory/details/level/watchlist/id/{id}\n" +
      "  • /v1/system/list/level/customers → /v1/system/inventory/details/level/customer/id/{id}\n\n" +
      "Possible causes:\n" +
      "  • Your API token does not have access to any customer fleet\n" +
      "  • Your watchlists have no systems assigned\n" +
      "  • Your account requires a specific customer/site ID\n\n" +
      "Check: activeiq.netapp.com → Quick Links → API Services → System Inventory"
    );
    updateStatusIndicators();
    return;
  }

  // Normalise field names — confirmed from API library model definitions
  const normalise = s => ({
    serialNumber:    s.serialNumber    || s.serial_number    || "",
    systemName:      s.systemName      || s.system_name      || s.hostName || s.hostname || "",
    model:           s.model           || s.platformType     || "",
    osVersion:       s.osVersion       || s.os_version       || s.ontapVersion || "",
    ...
    location:        s.location        || s.siteName         || "",
    platform:        s.platform        || s.platformType     || "",
    systemType:      s.systemType      || s.system_type      || "",
    productFamily:   s.productFamily   || s.product_family   || "",
    contractEndDate: s.contractEndDate || s.contract_end_date || "",
    riskCount:       s.riskCount       || s.risk_count       || 0,
    warningCount:    s.warningCount    || s.warning_count    || 0,
    criticalCount:   s.criticalCount   || s.critical_count   || 0,
    ...s  // keep any extra fields the API returns
  });

  // Apply watchlist serial filter if watchlist-only mode enabled
  if (state.watchlistOnly && watchlistSerials.size > 0 && watchlistIds.length === 0) {
    // Only filter by serials if we couldn't filter by watchlist ID at query time
    systemsList = systemsList.filter(s => {
      const sn = String(s.serialNumber || s.serial_number || "").trim();

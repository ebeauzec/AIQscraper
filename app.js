...
* ACTION 3.2: Enable SVM Configuration Change Auditing
  ...
  - Run Command: 'vserver audit create -vserver <svm_name> -destination /audit_log'
...
1. PRE-CHANGE VERIFICATION: Execute 'system health alert show' and 'storage failover show' to verify cluster stability.
2. MAINTENANCE WINDOWS: Schedule hardware spares replacement during off-peak windows to absorb transient latency spikes.
3. CHANGE ROLLBACK PLAN: Document rollback CLI commands for all upgrades (e.g. reverting to fallback boot partitions if post-upgrade checks fail).
================================================================================`;
}
...
- Cross-reference active shipment codes against tracking APIs.`;
  ...
  const customerSuccessPlanText = compileCustomerSuccessPlanText(scopeTitle, allRisks, allUpgrades, targetSystems, expiringContracts, allSupportCases);
...
- Cross-reference active shipment codes against tracking APIs.`;
    ...
    const text = compileCustomerSuccessPlanText(scopeTitle.replace(/_/g, ' '), allRisks, allUpgrades, targetSystems, expiringContracts, allSupportCases);

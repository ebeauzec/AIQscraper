...
async function runAPIDiagnostics() {
  ...
  try {
    ...
    results.push({ label: "─── Token ───", ep: "", status: "", ms: 0,
      preview: tok === "ERROR" ? "⚠ Token exchange failed!" : `Prefix: ${tok.slice(0,25)}…`, ok: tok !== "ERROR" });

    await probe("Watchlist All",              "/watchlist/all");
    await probe("Search System v3 (wildcard)", "/v3/search/system?searchText=*&limit=10");
    await probe("Search System v3 (blank)",    "/v3/search/system?limit=10");
    ...
    await probe("Upgrade Summary v1",          "/upgrade/summary/level/customer?searchText=*");
    ...
    if (realWatchlists.length > 0) {
      ...
      results.push({ label: "─── Watchlists ───", ep: "", status: "", ms: 0,
        preview: "No real watchlists loaded yet (run Sync first, or /watchlist/all returned error)", ok: false });
    }

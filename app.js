...
async function loadProductionData() {
  ...
  // ── Step 1: Fetch watchlists (/v1/watchlist/all) ──────────────────────────
  let watchlistSerials = new Set();
  let watchlistIds = [];
  try {
    if (textLabel) textLabel.innerText = "Loading Watchlists...";
    const apiWatchlists = await callActiveIQAPI("/watchlist/all");
    if (apiWatchlists) {
      const wlList = Array.isArray(apiWatchlists)
        ? apiWatchlists
        : (apiWatchlists.watchListData || apiWatchlists.watchlists || []);
      state.watchlists = wlList.map(wl => {
        ...
        watchlistIds.push(wl.watchListId || wl.watchlistId || wl.id);
        return {
      ...
      saveWatchlists();
      console.log(`[AIQ] Watchlists: ${state.watchlists.length} loaded, ${watchlistSerials.size} serials.`);
    }
  } catch (wlErr) {
    console.warn("[AIQ] /watchlist/all failed:", wlErr.message);
  }

  // ── Step 2: Fetch systems ─────────────────────────────────────────────────
  //
  // NetApp has several possible endpoints. We try them in order:
  //   a) /v1/system          — standard single-system or system-list endpoint
  //   b) /v1/system/list     — explicit list variant
  //   c) /v1/cluster         — cluster-based inventory
  // If Watchlist-Only mode is on and we got watchlist IDs, also try per-watchlist:
  //   d) /v1/watchlist/details/watchlistId/{id}
  //
  ...
  // 2a. Try standard REST inventory endpoints
  for (const ep of SYSTEM_ENDPOINTS) {
    try {
      const data = await callActiveIQAPI(ep);
      const parsed = Array.isArray(data) ? data
        : (data?.systems || data?.clusters || data?.storageSystemList || [data]);
      if (parsed.length > 0 && parsed[0].serialNumber !== undefined) {
        systemsList = parsed;
        usedEndpoint = ep;
        console.log(`[AIQ] Systems found via REST ${ep}: ${parsed.length}`);
        break;
      }
    } catch (e) {
      console.warn(`[AIQ] ${ep} → ${e.message}`);
    }
  }

  // 2b. If no systems yet and we have watchlist IDs, try per-watchlist system endpoint
  if (systemsList.length === 0 && watchlistIds.length > 0) {
    console.log("[AIQ] Trying watchlist-scoped system endpoints...");
    const WL_PATTERNS = [
      id => `/watchlist/systems/watchlistId/${id}`,
      id => `/watchlist/details/watchlistId/${id}`,
      id => `/watchlist/${id}/systems`,
    ];
    outer: for (const id of watchlistIds.filter(Boolean)) {
      for (const pattern of WL_PATTERNS) {
        try {
          const data = await callActiveIQAPI(pattern(id));
          const parsed = Array.isArray(data) ? data
            : (data?.systems || data?.storageSystemList || []);
          if (parsed.length > 0) {
            systemsList = [...systemsList, ...parsed];
            usedEndpoint = pattern(id);
            console.log(`[AIQ] Systems via watchlist ${id}: ${parsed.length}`);
            break outer;
          }
        } catch (e) {
          console.warn(`[AIQ] watchlist ${id} pattern failed: ${e.message}`);
        }
      }
    }
  }
  ...
  if (systemsList.length === 0) {
    ...
    // Build a helpful diagnostic message listing what worked/failed
    const viaProxy = state.isRunningViaProxy;
    alert(
      "Active IQ API connected successfully but returned no systems.\n\n" +
      "Endpoints tried: " + SYSTEM_ENDPOINTS.join(", ") + "\n\n" +
      "This usually means one of:\n" +
      "  • The system list requires a different endpoint path\n" +
      "  • Your API token does not have fleet/system-list access\n" +
      "  • Watchlist-Only mode is active but no watchlists contain systems\n\n" +
      "To diagnose: look up the system list endpoint in the Active IQ API Catalog\n" +
      "(activeiq.netapp.com → Quick Links → API Services)\n" +
      "and paste the path into the API Base URL override in Settings."
    );
    updateStatusIndicators();
    return;
  }

  // Normalise field names (API uses camelCase; enrichment handles snake_case too)
  const normalise = s => ({
    serialNumber:    s.serialNumber    || s.serial_number    || "",
    systemName:      s.systemName      || s.system_name      || s.name || "",
    model:           s.model           || "",
    osVersion:       s.osVersion       || s.os_version       || "",
    ...
    ...s  // keep any extra fields the API sends
  });

  // Apply watchlist filter if enabled
  if (state.watchlistOnly && watchlistSerials.size > 0) {
  ...
  const seen = new Set();
  systemsList = systemsList.filter(s => {
    const sn = (s.serialNumber || s.serial_number || "").trim();
  ...
  refreshUIState();
    ...
    if (apiWatchlists) {
      // API returns: { watchListData: [...] } or an array directly
      const wlList = Array.isArray(apiWatchlists)
        ? apiWatchlists
        : (apiWatchlists.watchListData || apiWatchlists.watchlists || []);
      state.watchlists = wlList.map(wl => {
        // Field names vary between API versions
        const serials = wl.serialNumbers || wl.systemSerials || wl.systems || [];
        serials.forEach(sn => watchlistSerials.add(sn.toString().trim()));
        return {
          id:            wl.watchListId  || wl.watchlistId  || wl.id  || "wl_" + Date.now(),
      ...
      saveWatchlists();
      console.log(`[AIQ] Watchlists loaded: ${state.watchlists.length}`);
    }
  } catch (wlErr) {
    console.warn("[AIQ] /watchlist/all failed:", wlErr.message);
  }

  // 2. Fetch system inventory via GraphQL
  //    NetApp has migrated most REST inventory endpoints to GraphQL.
  //    The query below fetches all storage systems visible to the authenticated user.
  try {
    const inventoryQuery = `
      query GetSystemInventory {
        systemInventory {
          serialNumber
          systemName
          model
    ...
    `;
    const gqlData = await callActiveIQGraphQL(inventoryQuery);
    const rawSystems = gqlData?.systemInventory || [];

    // Fall back to REST /v1/system if GraphQL returns nothing
    let systemsList = rawSystems;
    if (systemsList.length === 0) {
      console.warn("[AIQ] GraphQL returned no systems, trying REST /v1/system...");
      try {
        const restData = await callActiveIQAPI("/system");
        systemsList = Array.isArray(restData) ? restData : (restData?.systems || []);
      } catch (restErr) {
        console.warn("[AIQ] REST /v1/system also failed:", restErr.message);
      }
    }

    if (systemsList.length === 0) {
      console.warn("[AIQ] Active IQ API returned no systems.");
      alert("Warning: The Active IQ API connected successfully, but returned no systems.\n\nThis may mean:\n• Your token doesn't have fleet access\n• All systems are filtered by watchlist\n• The API account has no associated systems");
      return;
    }

    // Normalise field names — the API uses camelCase; our enrichment function
    // accepts both camelCase and snake_case, but let's map any stray fields here.
    const normalise = s => ({
      serialNumber:    s.serialNumber    || s.serial_number    || "",
      systemName:      s.systemName      || s.system_name      || s.name || "",
      model:           s.model           || "",
    ...
    // If Watchlist-Only filter is active, restrict to known serials
    if (state.watchlistOnly && watchlistSerials.size > 0) {
      systemsList = systemsList.filter(s => {
        const sn = (s.serialNumber || s.serial_number || "").toString().trim();
        return watchlistSerials.has(sn);
    ...
    safeSetItem("aiq_last_sync", state.lastSync);
    if (state.systems.length > 0) state.selectedSystem = state.systems[0];
    console.log(`[AIQ] Sync complete: ${state.systems.length} systems loaded.`);
    updateStatusIndicators();
    refreshUIState();
    return;
  } catch (error) {

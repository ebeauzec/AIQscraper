...
async function runAPIDiagnostics() {
  ...
  try {
    ...
    await probe(`Cluster resolver: ${diagSerial}`,  `/v1/clusterview/resolver/${diagSerial}`);
    await probe(`Health summary: ${diagSerial}`,    `/v1/health/summary/level/system/id/${diagSerial}`);
    await probe(`Health details: ${diagSerial}`,    `/v1/health/details/level/system/id/${diagSerial}`);
    await probe(`EoS by serial: ${diagSerial}`,     `/eos/details?serialNumbers=${diagSerial}`);
    await probe(`Upgrade by serial: ${diagSerial}`, `/upgrade/details/level/system/id/${diagSerial}`);
...
async function loadProductionData() {
  ...
  // 3d. Breadth endpoints (return data for ALL accessible systems — no ID needed)
  if (systemsList.length === 0) {

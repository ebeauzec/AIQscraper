...
def harvest_imt_versions(existing_imt):
    ...
    # Third-party vendor version scrapes (Veeam, Commvault, Veritas, vSphere,
    # Hyper-V, RHV, OpenStack, Citrix, Proxmox, Nutanix) were removed: they
    # regex-matched the first version-looking number on marketing/doc landing
    # pages and got it wrong (Veeam "8.1" vs real 12.3, vSphere "7.0"), several
    # pages 401/403 bot traffic or are for retired products (RHV), and the
    # bad values overwrote the curated currentRecommended in imt_interop.json
    # -- which made runIMTInteropCheck silently skip Veeam/Commvault because
    # the scraped version wasn't a key in its version table. Nine of the ten
    # weren't in the interop matrix at all, so their values fed nothing.
    # Only NetApp-owned sources (docs.netapp.com, NetApp GitHub, PyPI) are
    # harvested above.
